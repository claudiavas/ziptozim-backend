...
routes.post('/upload', upload.single('inputFile'), async (req, res, next) => {
    try {
        ...
        const sourceDirectory = path.join(__dirname, '/temp', String(timestamp));
        fs.mkdirSync(sourceDirectory, { recursive: true }); // Ensure the directory exists
        await unzipFile(req.file, sourceDirectory);
        console.log("source Directory", sourceDirectory)
        res.status(200).send({ message: 'File uploaded and unzipped successfully.', sourceDirectory });
    } catch (err) {
        console.error(err);
        res.status(500).send({ message: 'Error uploading and unzipping the file.', error: err.message });
    }
});
...
routes.post('/convert', 
    ...
    async (req, res) => {
        ...
        try {
            const { sourceDirectory, welcomePage, favicon, language, title, description, creator, publisher } = req.body;
            ...
            await createZimFile(sourceDirectory, zimStream, escapeHtml(welcomePage), escapeHtml(favicon), escapeHtml(language), escapeHtml(title), escapeHtml(description), escapeHtml(creator), escapeHtml(publisher));
            ...
            console.error(err);
            res.status(500).send({ message: 'Error converting the file.', error: err.message });
        }
    }
);
